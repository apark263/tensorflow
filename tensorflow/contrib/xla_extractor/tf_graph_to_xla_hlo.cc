...
void RealMain(TfToXlaConverterOptions converter_options) {
  ...
  Status compile_status =
      FindAndCompileLaunchNodes(g, converter_options.target_node, &xla_modules);
  std::cout<<"step 2\n";
  ...
  std::cout<<"size: "<<xla_modules.size()<<"\n";
  for (unsigned int idx = 0; idx < xla_modules.size(); ++idx) {
    ...
    std::cout<<out_filename.str()<<" apple\n";
    Status save_status =
...
int main(int argc, char** argv) {
  ...
  std::vector<tensorflow::Flag> flag_list = {
      ...
      {"out_prefix", &converter_options.out_prefix,
       "prefix for output xla hlo modules protobufs.  Names will be "
       "<prefix>_<i>.pb(txt).  Default is \"xla_graph\"."},
