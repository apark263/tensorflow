...
bool is_target_node_op(StringPiece node_op) {
  ...
  if (node_op.find("Resource")) return true;
  if (node_op.find("Dummy")) return true;
  if (node_op == "NoOp") return true;
  if (node_op == "ControlTrigger") return true;
  if (node_op.find("Assign")) return true;
  return false;
}
...
Status FindAndCompileLaunchNodes(
  ...
  std::cout<<graphs_list.size()<<" mango\n";
  for (Graph* partition_graph : graphs_list) {
    GraphDef inter_graph_def;
    partition_graph->ToGraphDef(&inter_graph_def);
    std::ofstream myfile;
    myfile.open ("graph_prefinal.pbtxt");
    myfile << inter_graph_def.DebugString();
    myfile.close();
    for (const Node* node : partition_graph->op_nodes()) {
      if (node->type_string() == "_XlaCompile"/*"XlaLaunch"*/) {
        VLOG(2) << "LaunchNode\n" << SummarizeNode(*node);

        s = CompileLaunchNode(compiler, *node, xla_modules);
        std::cout<<"counter: "<<xla_modules->size()<<"\n";
        if (!s.ok()) return s;
      }
    }
  }
